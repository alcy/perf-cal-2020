...
First, we define some global vectors for the model in R
...
Then, we define the AWS-Tomcat instance model using PDQ functions
...
Loop over the load generated by mobile requests
...
Finally, we plot throughput X(N), which corresponds to the white squares in Fig. 1.
```R
plot(xx, yx, 
